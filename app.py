# app.py — AI Resume Analyzer (Azure-ready Streamlit)
...
# ---------- Optional OCR deps (installed only if available) ----------
try:
...
def read_cfg(name: str, default: str = "") -> str:
    """Priority: OS/.env → st.secrets → default."""
    val = os.getenv(name)
    if val:
        return val
    if _has_local_secrets_file():
        try:
...
TESSERACT_CMD  = read_cfg("TESSERACT_CMD")      # /usr/bin/tesseract in Azure
POPPLER_PATH   = read_cfg("POPPLER_PATH")       # /usr/bin in Azure

if pytesseract and TESSERACT_CMD:
    pytesseract.pytesseract.tesseract_cmd = TESSERACT_CMD  # type: ignore

if not OPENAI_API_KEY or not OPENAI_API_KEY.startswith("sk-"):
    st.error("❌ Missing or invalid OPENAI_API_KEY. Set it in Azure App Settings or Streamlit Secrets.")
    st.stop()
...
def ocr_runtime_available() -> Tuple[bool, str]:
    ...
    if not has_poppler:
        return False, "Poppler binary not found."
...
def parse_pdf(pdf_bytes: bytes) -> str:
    ...
    try:
                ...
                if not t:  # OCR fallback
                    ...
                    if ok and convert_from_bytes and pytesseract:
                        try:
                            ...
                            if images:
                                used_ocr = True
                                t = pytesseract.image_to_string(images[0], config="--oem 3 --psm 6")  # type: ignore
                        except Exception:
                            pass
                    else:
        ...
        st.warning("⚠️ Could not open PDF (encrypted or corrupted).")
    text = _clean("\n".join(parts))
    if used_ocr:
        st.caption("🔍 OCR was used for one or more pages.")
    return text
...
def parse_image(img_bytes: bytes) -> str:
    ...
    if pytesseract and ocr_runtime_available()[0]:
        try:
            return _clean(pytesseract.image_to_string(img, config="--oem 3 --psm 6"))  # type: ignore
        except Exception:
            return ""
    return ""
...
def suggest(resume: str, jd: str, score: float, skills: List[str]) -> str:
    system = "You are a precise resume coach. Be concise, use bullet points, give concrete examples."
...
Tasks:
1) 2-line summary of fit.
2) Top 8 missing/weak areas.
3) Rewrite 4 resume bullets with quantified impact verbs tailored to the JD.
4) 5-item action plan (courses/projects) for the next 2 weeks."""
...
st.caption("Upload Resume + JD • Embeddings + Cosine Similarity • GPT Suggestions")
...
with st.sidebar:
    ...
    st.markdown("Troubleshooting: set **POPPLER_PATH** and **TESSERACT_CMD** in Azure App Settings.")
...
if f_res: resume_txt = parse_any(f_res.read(), f_res.name)
if f_jd:  jd_txt = parse_any(f_jd.read(), f_jd.name)
